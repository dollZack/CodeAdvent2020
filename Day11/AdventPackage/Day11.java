...
// import OccupiedSearch;
public class Day11 {

    public static void printSeats(ArrayList<String[]> chart) {
    ...
    public static ArrayList<String[]> chartParser(Scanner input_scan) {
    ...
    private static ArrayList<String[]> rearrange2(ArrayList<String[]> seating_chart) {
        ArrayList<String[]> updated_chart = chartCloner(seating_chart);
        ...
        for (int i = 0; i < seating_chart.size(); i++) {
            ...
            for (int j = 0; j < row.length; j++) { // for every chair in this row
                ...
                int occupied = occupiedVisible(seating_chart, i, j);

                if (row[j].equals("L") && occupied == 0) {
                    row[j] = "#";
                    // changed_seats++;
                } else if (row[j].equals("#") && occupied >= 5) {
                    row[j] = "L";
                    // changed_seats++;
                } else {
                    //nothing
     ...
     static int occupiedVisible(ArrayList<String[]> seating_chart, int i, int j) {
        int occupied = 0;

        // look above
        if (i > 0) {
            // diagonal left
            if (OccupiedSearch.upLeft(seating_chart, i, j)) { occupied++; }
            // up
            if (OccupiedSearch.above(seating_chart, i, j)) { occupied++; }
            // diagonal right 
            if (OccupiedSearch.upRight(seating_chart, i, j)) { occupied++; }
        }

        // look adjacent
            // left
            if (OccupiedSearch.left(seating_chart, i, j)) { occupied++; }
            // right
            if (OccupiedSearch.right(seating_chart, i, j)) { occupied++; }

        // look below
        if (i < seating_chart.size()) {
            // diagonal left
            if (OccupiedSearch.downLeft(seating_chart, i, j)) { occupied++; }
            // down
            if (OccupiedSearch.below(seating_chart, i, j)) { occupied++; }
            // diagonal right
            if (OccupiedSearch.downRight(seating_chart, i, j)) { occupied++; }
        }
    ...
    /**
     * Takes the seating chart and an entry and counts the occupied adjacent seats 
     * @param seating_chart
     * @param i
     * @param j
     * @return Count of adjacent seats
     */
    ...
    /**
     * Compares the two given charts for any differences in entry values.
     * @param prev_chart
     * @param new_chart
     * @return true if there are differences, false if there aren't
     */
    ...
    /**
     * Counts the number of occupied seats in the given chart.
     * @param chart
     * @return The count of occupied seats.
     */
    ...
    private static void seatingSystem(Scanner input_scan) {
        ...
        // for (int i = 0; i < 6; i++) {
            ...
            seating_chart = rearrange2(seating_chart);
            //DEBUG
